...
--Highest death count recorded by continent
...
FROM (
    ...
    FROM ProjectCovid.dbo.CovidDeaths_Clean
    WHERE continent IS NOT NULL   -- 🔥 this is key
    AND location NOT IN ('World', 'Europe', 'North America', 'Asia', 'Africa', 'South America', 'European Union', 'International', 'Oceania')
...
--This one shows my continent numbers are true
SELECT 
    location, 
    MAX(total_deaths) AS HighestDeathCount
FROM ProjectCovid.dbo.CovidDeaths_Clean
...
--CTE Common Table Expression. Calculate RollingVac inside CTE then calculate % in the outer query
...
-- Outer query
SELECT *,
ROUND((RollingVaccinations * 100.0) / NULLIF(population, 0), 2) AS PopulationPercentVaccinated
FROM PopVsVac
